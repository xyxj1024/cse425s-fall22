...
(* expressions in a little language for 2D geometry objects
   ...
   other expressions: intersection of two expressions, lets, variables, shifts
*)
datatype geom_exp = 
  NoPoints
...
| Var of string                            (* var (string) for string lookup in the environment *)
...
(* notice curried *)
fun real_close_point (x1, y1) (x2, y2) = real_close (x1, x2) andalso real_close (y1, y2)
	...
	(* the hard case, actually 4 cases because v2 could be a point,
      ...
      | Point (x0, y0) => (* see if the point is within the segment bounds *)
          ...
          let 
              ...
              (end1 - epsilon <= v andalso v <= end2 + epsilon) orelse
		          (end2 - epsilon <= v andalso v <= end1 + epsilon)
		        val (x1,y1,x2,y2) = seg
	        in
